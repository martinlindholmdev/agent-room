...
def identify(claude_channel=False, claude_app=False, generic=False, native_arg=None):
    """Derive (app, native identity) from the selected mode. Exactly one mode may be set.

    Generic mode is self-identified: the host supplies its own native id (env
    AGENT_ROOM_NATIVE, or --native) instead of a Claude- or Codex-specific env var.
    """
    if sum((bool(claude_channel), bool(claude_app), bool(generic))) > 1:
        raise SystemExit('Select only one of Claude channel push, Claude app read-on-demand, or generic MCP.')
    if generic:
        return 'mcp', native_arg or os.environ.get('AGENT_ROOM_NATIVE')
    return ('claude-channel' if claude_channel else 'pull' if claude_app else 'codex-queue',
            os.environ.get('CLAUDE_CODE_SESSION_ID' if claude_channel or claude_app else 'CODEX_THREAD_ID'))


def resolve_binding(bindings, identity=None, claude_channel=False, claude_app=False, generic=False, native_arg=None):
    app, native = identify(claude_channel, claude_app, generic, native_arg)
...
def room_connect(root, claude_channel=False, claude_app=False, title='', model='', generic=False, native_arg=None):
...
def run(root, identity, claude_channel=False, claude_app=False, generic=False, native_arg=None):
    snapshot=local_call(root,'snapshot',{})
    try:
        binding=resolve_binding(snapshot['bindings'],identity,claude_channel,claude_app,generic,native_arg)
    except SystemExit:
    ...
    def try_activate():
        ...
        if binding is not None:
            return True
        snapshot=local_call(root,'snapshot',{})
        app, _ = identify(claude_channel, claude_app, generic, native_arg)
            ...
            if method == 'initialize':
                ...
                if params['name'] == 'room_connect':
                    ...
                    if binding is not None:
                        ...
                        value = room_connect(root, claude_channel, claude_app, arguments.get('title', ''), arguments.get('model', ''), generic, native_arg)
